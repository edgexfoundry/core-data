...
public class ReadingRepositoryTest {
	...
	/**
	 * Create and save an instance of the Reading before each test Note: the
	...
	public void testUpdate() {
		...
		// Reading record
		assertEquals("Modified and created timestamps should be equal after creation", reading.getModified(),
				reading.getCreated());
		reading.setName("newname");
		readingRepos.save(reading);
		// reread reading
		Reading reading2 = readingRepos.findOne(testReadingId);
		assertEquals("name was not updated appropriately in reading update", "newname", reading2.getName());
		assertNotEquals("after modification, modified timestamp still the same as the reading's create timestamp",
	...
	public void testFindByNameAndValueWithNoMatching() {
		...
		assertTrue("find by name and value with bad name/value is returning readings", readings.isEmpty());
	}
	...
	public void testFindByNameAndDeviceWithNoMatching() {
		...
		assertTrue("find by name and device with bad name/device is returning readings", readings.isEmpty());
	}
	...
	private void checkReadingData(Reading reading) {
		...
		assertEquals("Reading pushed does not match saved pushed date", TEST_PUSHED, reading.getPushed());
		assertEquals("Reading device does not match saved device", EventData.TEST_DEVICE_ID, reading.getDevice());
