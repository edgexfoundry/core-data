...
public class ValueDescriptorRepositoryTest {
	...
	/**
	 * Create and save an instance of the ValueDescriptor before each test Note:
	...
	public void cleanup() {
		...
		assertNull("ValueDescriptor not deleted as part of cleanup", valDescRepos.findOne(testValDescId));
	}
	...
	public void testUniqueNameFail() {
		...
		// non-unique name
		ValueDescriptor valDesc2 = new ValueDescriptor(TEST_NAME, TEST_MIN, TEST_MAX, TEST_TYPE, TEST_UOMLABEL,
				TEST_DEF_VALUE, TEST_FORMATTING, TEST_LABELS, TEST_DESCRIPTION);
	...
	public void testUpdate() {
		...
		assertEquals("Modified and created timestamps should be equal after creation", valueDescriptor.getModified(),
				valueDescriptor.getCreated());
		...
		assertEquals("min was not updated appropriately in ValueDescriptor update", -200, valueDescriptor2.getMin());
		assertNotEquals(
	...
	public void testFindAll() {
		...
		assertEquals("Find all not returning a list with one value descriptor", 1, valDescriptors.size());
	...
	public void testFindByUomLabel() {
		...
		assertEquals("find by UOM label not returning any value descriptors", 1, valueDescriptors.size());
	...
	public void testFindByUomLabelWithNoMatching() {
		...
		assertTrue("find by uom label with bad uom label is returning value descriptors", valueDescriptors.isEmpty());
	}
	...
	public void testFindByLabelWithNoMatching() {
		...
		assertTrue("find by label with bad label is returning value descriptors", valueDescriptors.isEmpty());
	}
	...
	public void testFindByTypeWithNoMatching() {
		...
		assertTrue("find by type with bad type is returning value descriptors", valueDescriptors.isEmpty());
	}
