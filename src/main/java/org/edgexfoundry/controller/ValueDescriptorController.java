...
public class ValueDescriptorController {
	...
	// Logger.getLogger(ValueDescriptorController.class);
	private final static org.edgexfoundry.support.logging.client.EdgeXLogger logger = 
			org.edgexfoundry.support.logging.client.EdgeXLoggerFactory.getEdgeXLogger(ValueDescriptorController.class);
	...
	/**
	 * Fetch a specific ValueDescriptor by its database generated id.
	 * NotFoundException (HTTP 404) if no value descriptor can be found by the
	 * id provided. ServcieException (HTTP 503) for unknown or unanticipated
	 * issues
	 * 
	 * @param String
	 *            ValueDescriptor id (ObjectId)
	 * 
	...
	/**
	 * Return all ValueDescriptor objects. LimitExceededException (HTTP 413) if
	 * the number of value descriptors exceeds the current max limit.
	 ...
	 *             (HTTP 413) if the number of value descriptors exceeds the
	 *             current max limit
	 */
	...
	/**
	 * Return ValueDescriptor object with given name. ServcieException (HTTP
	 * 503) for unknown or unanticipated issues.
	 * 
	 * @param name
	 *            of the value descriptor to locate and return
	 * @return ValueDescriptor having the provided name, could be null if none
	 *         are found.
	 * @throws ServcieException
	 *             (HTTP 503) for unknown or unanticipated issues
	 */
	...
	/**
	 * Return ValueDescriptor objects with given UoM label. ServcieException
	 * (HTTP 503) for unknown or unanticipated issues.
	 * 
	 * @param uomLabel
	...
	/**
	 * Return ValueDescriptor objects with given label. ServcieException (HTTP
	 * 503) for unknown or unanticipated issues.
	 * 
	 * @param label
	 * @return list of ValueDescriptor matching UoM Label
	...
	/**
	 * Retrieve value descriptors associated to a device where the device is
	 * identified by name - that is value descriptors that are listed as part of
	 * a devices parameter names on puts or expected values on get or put
	 * commands. Throws a ServcieException (HTTP 503) for unknown or
	 * unanticipated issues. Throws NotFoundExcetption (HTTP 404) if a device
	 * cannot be found for the name provided.
	 * 
	 * @param name
	 *            - name of the device
	 ...
	 *             (HTTP 404) for device not found by name
	 */
	...
	public List<ValueDescriptor> valueDescriptorsForDeviceByName(@PathVariable String name) {
		try {
			...
			Set<String> vdNames = d.getProfile().getCommands().stream()
					.map((Command c) -> c.associatedValueDescriptors()).flatMap(l -> l.stream())
					.collect(Collectors.toSet());
			return vdNames.stream().map(s -> this.valDescRepos.findByName(s)).collect(Collectors.toList());
	...
	/**
	 * Retrieve value descriptors associated to a device where the device is
	 * identified by id - that is value descriptors that are listed as part of a
	 * devices parameter names on puts or expected values on get or put
	 * commands. Throws a ServcieException (HTTP 503) for unknown or
	 * unanticipated issues. Throws NotFoundExcetption (HTTP 404) if a device
	 * cannot be found for the id provided.
	 * 
	 * @param name
	 *            - name of the device
	 * @return list of value descriptors associated to the device.
	 ...
	 *             (HTTP 503) for unknown or unanticipated issues
	 * @throws NotFoundException
	 *             (HTTP 404) for device not found by id
	 */
	...
	/**
	 * Add a new ValueDescriptor whose name must be unique. ServcieException
	 * (HTTP 503) for unknown or unanticipated issues. DataValidationException
	 * (HTTP 409) if the a formatting string of the value descriptor is not a
	 ...
	 *             (HTTP 409) if the format string is not valid or name not
	 *             unique
	 */
	...
	public String add(@RequestBody ValueDescriptor valueDescriptor) {
		...
		try {
			...
			throw new DataValidationException("Value descriptor's name is not unique: " + valueDescriptor.getName());
		} catch (Exception e) {
	 ...
	 * purposes. ServcieException (HTTP 503) for unknown or unanticipated
	 * issues. DataValidationException (HTTP 409) if the a formatting string of
	 * the value descriptor is not a valid printf format. NotFoundException
	 * (404) if the value descriptor cannot be located by the identifier.
	 * 
	 * @param valueDescriptor2
	 ...
	 *             (HTTP 503) for unknown or unanticipated issues
	 * @throws DataValidationException
	 *             (HTTP 409) if the format string is not valid
	 * @throws NotFoundException
	 *             (HTTP 404) when the value descriptor cannot be found by the
	 *             id
	 */
	@RequestMapping(method = RequestMethod.PUT)
	public boolean update(@RequestBody ValueDescriptor valueDescriptor2) {
		try {
			ValueDescriptor valueDescriptor = getValueDescriptorByIdOrName(valueDescriptor2.getId(),
					valueDescriptor2.getName());
			if (valueDescriptor != null) {
				updateValueDescriptor(valueDescriptor2, valueDescriptor);
				return true;
			} else {
				logger.error("Request to update with non-existent or unidentified value descriptor (id/name):  "
	...
	/**
	 * Remove the ValueDescriptor designated by database generated identifier.
	 * ServcieException (HTTP 503) for unknown or unanticipated issues.
	 * DataValidationException (HTTP 409) if the value descriptor is still
	 * referenced in Readings. NotFoundException (404) if the value descriptor
	 * cannot be located by the identifier.
	 * 
	 * @param identifier
	 *            (database generated) of the value descriptor to be deleted
	 * @return boolean indicating success of the remove operation
	 * @throws ServcieException
	 *             (HTTP 503) for unknown or unanticipated issues
	 * @throws DataValidationException
	 *             (HTTP 409) if the value descriptor is still referenced by
	 *             readings
	 * @throws NotFoundException
	 *             (HTTP 404) when the value descriptor cannot be found by the
	 *             id
	...
	/**
	 * Remove the ValueDescriptor designated by name. ServcieException (HTTP
	 * 503) for unknown or unanticipated issues. DataValidationException (HTTP
	 * 409) if the value descriptor is still referenced in Readings.
	 * NotFoundException (404) if the value descriptor cannot be located by the
	 * identifier.
	 * 
	 * @param name
	 *            of the value descriptor to be removed.
	 * @return boolean indicating success of the remove operation
	 * @throws ServcieException
	 *             (HTTP 503) for unknown or unanticipated issues
	 * @throws DataValidationException
